...
ITER = 1
CATEGORY = 16
train_indices = np.load("/home/asdf/Documents/juyan/paper/salinas/cnn/data/labeled_index.npy")
test_indices = np.load("/home/asdf/Documents/juyan/paper/salinas/cnn/data/test_index.npy")
val_indices = np.load("/home/asdf/Documents/juyan/paper/salinas/cnn/data/valid_index.npy")
train_data = np.zeros((len(train_indices), 2*PATCH_LENGTH + 1, 2*PATCH_LENGTH + 1, INPUT_DIMENSION_CONV))
test_data = np.zeros((len(test_indices), 2*PATCH_LENGTH + 1, 2*PATCH_LENGTH + 1, INPUT_DIMENSION_CONV))
val_data = np.zeros((len(val_indices), 2*PATCH_LENGTH + 1, 2*PATCH_LENGTH + 1, INPUT_DIMENSION_CONV))
...
for index_iter in xrange(ITER):
    ...
    best_weights_RES_path_ss4 = '/home/asdf/Documents/juyan/SSRN-master/juyan/model/SA_'+str(index_iter+1)+'.hdf5'
...
#    train_indices, test_indices = sampleFixNum.samplingFixedNum(TRAIN_NUM, gt)
    ...
    for i in range(len(val_assign)):
        val_data[i] = selectNeighboringPatch(padded_data, val_assign[i][0], val_assign[i][1], PATCH_LENGTH)
    print("val ok")
    x_train = train_data.reshape(train_data.shape[0], train_data.shape[1], train_data.shape[2],INPUT_DIMENSION_CONV)
    x_test = test_data.reshape(test_data.shape[0], test_data.shape[1], test_data.shape[2], INPUT_DIMENSION_CONV)
    x_val = val_data.reshape(val_data.shape[0], val_data.shape[1], val_data.shape[2], INPUT_DIMENSION_CONV)
    ...
    loss_and_metrics_res4_SS = model_res4_SS_BN.evaluate(
        x_val.reshape(x_val.shape[0], x_val.shape[1], x_val.shape[2], x_val.shape[3], 1), y_val,
        batch_size=batch_size)
...
#     # np.save('/home/asdf/Documents/juyan/SSRN-master/juyan/result/pavia.npy', pred_test_res4)
